...
Scheduler가 정상적으로 실행되지 않으면 워크플로우 자체가 시작되지 않는다.
...
즉, 전체 자동화 프로세스가 수행되지 않게 된다.

따라서 Scheduler는 전체 워크플로우를 시작하는 핵심 트리거 역할을 수행한다.
...
3줄 제한은 후처리(Post Processing)가 아닌 **Gemini 프롬프트 단계에서 직접 강제하였다.**

프롬프트에 다음과 같은 조건을 명시하였다.

- 반드시 핵심 내용만 작성
- 최대 3개의 Bullet 형태로 출력
- 불필요한 설명 및 수식어 제외
- 지정된 JSON 구조를 유지하여 반환

Gemini가 생성한 JSON 결과는 Parse JSON 모듈을 통해 구조화되며, 이후 그대로 Notion에 저장된다.

이를 통해 모든 뉴스가 동일한 형식과 길이로 저장되도록 구성하였다.
