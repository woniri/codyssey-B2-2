...
![Discord 오류 알림](images/discord_error.png)

### 검증 포인트

- 오류 발생 시 Discord 채널로 즉시 알림 전송
- 오류 발생 모듈 확인 가능
- 오류 메시지 확인 가능
- 발생 시간 확인 가능
...
워크플로우 실행 중 주요 모듈에서 오류가 발생하는 경우 Discord 채널로 오류 정보가 정상적으로 전송되는 것을 확인하였다.

이를 통해 자동화 시스템 운영 중 발생하는 문제를 즉시 확인하고 대응할 수 있도록 구현하였다.
